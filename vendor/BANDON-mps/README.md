...
This fork adds inference support for Apple Silicon Macs by routing BANDON / MTGCDNet inference through PyTorch MPS when CUDA is unavailable.

Key points:

- CUDA remains preferred when available
- MPS is selected automatically on Apple Silicon when supported
- CPU fallback remains available
- the patched inference entrypoint is `tools/infer_mps.py`
- setup and verification details are documented in [MACOS_MPS.md](./MACOS_MPS.md)

This fork is inference-focused. It preserves the original project attribution and keeps the patch scoped to device-agnostic inference compatibility.
