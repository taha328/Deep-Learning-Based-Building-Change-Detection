...
import { getCachedRunResponse, getTemporalProject, listTemporalProjects, runDetection, validateRequest } from "@/api/gradio";
...
import { saveTemporalProject } from "@/api/gradio";
import {
  PIPELINE_STAGES,
...
export function SettingsPanel({
  ...
  const downloadBundle = async () => {
    ...
    await downloadFileFromUrl(buildGradioFileUrl(backendUrl, result.downloadable_zip_path), name);
  };

  const downloadArtifact = async (path: string, name: string) => {
    await downloadFileFromUrl(buildGradioFileUrl(backendUrl, path), name);
  };
